...
class GenerateBillingReport extends Command
{
     ...
     * @var string
     */
    protected $signature = 'data:generate_billing_report {date?}';
    ...
    public function handle()
    {
        ...
        BetLog::select(
            ...
            ->where('bet_logs.account_date', $date)
            ...
            ->chunk($chunkSize, function ($betLogs) use ($date) {
                foreach ($betLogs as $betLog) {
                    ...
                    $uniqueKey = $date . "_" . $memberId . "_" . $productId;
                    ...
                    $this->outputs[$uniqueKey]['wager'] += $betLog->bet_count;
                    $this->outputs[$uniqueKey]['turnover'] += $betLog->stake;
                    $this->outputs[$uniqueKey]['realbets'] += $betLog->valid_stake;
                    $this->outputs[$uniqueKey]['profit_loss'] += $betLog->winlose;
                    $this->outputs[$uniqueKey]['jackpot_win'] += $betLog->jackpot_win;
                    $this->outputs[$uniqueKey]['progressive_share'] += $betLog->progressive_share;
                }
            });

            ProductReportBilling::upsert($this->outputs, ['unique_key']);
